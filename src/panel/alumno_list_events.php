...
function BindEvents()
{
    ...
    $alumno->activos->CCSEvents["BeforeShow"] = "alumno_activos_BeforeShow";
...
//alumno_activos_BeforeShow @61-5225D549
function alumno_activos_BeforeShow(& $sender)
{
    $alumno_activos_BeforeShow = true;
    $Component = & $sender;
    $Container = & CCGetParentContainer($sender);
    global $alumno; //Compatibility
//End alumno_activos_BeforeShow

//Custom Code @62-2A29BDB7
// -------------------------
    $db = new clsDBConnection2();
	$sql = "select count(*) as activo from alumno where activo = 1";
	//var_dump($sql);
	$db->query($sql);
	$db->next_record();
	$alumno->activos->SetValue($db->f('activo'));
// -------------------------
//End Custom Code

//Close alumno_activos_BeforeShow @61-C00C04E4
    return $alumno_activos_BeforeShow;
}
//End Close alumno_activos_BeforeShow
