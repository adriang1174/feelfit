...
//Class_Initialize Event @2-F5FF0F72
...
"FROM alumno {SQL_Where} {SQL_OrderBy}";
            ...
            $this->s_fdesde = & new clsControl(ccsTextBox, "s_fdesde", "s_fdesde", ccsDate, array("yyyy", "-", "mm", "-", "dd"), CCGetRequestParam("s_fdesde", $Method, NULL), $this);
            $this->s_fhasta = & new clsControl(ccsTextBox, "s_fhasta", "s_fhasta", ccsDate, array("yyyy", "-", "mm", "-", "dd"), CCGetRequestParam("s_fhasta", $Method, NULL), $this);
...
//Prepare Method @5-DC2A707A
    function Prepare()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        $this->wp = new clsSQLParameters($this->ErrorBlock);
        $this->wp->AddParameter("1", "urls_fdesde", ccsDate, array("yyyy", "-", "mm", "-", "dd"), array("yyyy", "-", "mm", "-", "dd"), $this->Parameters["urls_fdesde"], "", false);
        $this->wp->AddParameter("2", "urls_fhasta", ccsDate, array("yyyy", "-", "mm", "-", "dd"), array("yyyy", "-", "mm", "-", "dd"), $this->Parameters["urls_fhasta"], "", false);
        $this->wp->AddParameter("3", "urls_alumno", ccsInteger, "", "", $this->Parameters["urls_alumno"], 0, false);
    }
//End Prepare Method

//Open Method @5-B668D60E
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->CountSQL = "SELECT COUNT(*) FROM asistenciaalumno INNER JOIN alumno ON(alumno.id = asistenciaalumno.idAlumno)\n" .
        ...
        "WHERE asistenciaalumno.fecha >= '" . $this->SQLValue($this->wp->GetDBValue("1"), ccsDate) . "'\n" .
        "AND asistenciaalumno.fecha <= '" . $this->SQLValue($this->wp->GetDBValue("2"), ccsDate) . "'\n" .
        "AND alumno.id = " . $this->SQLValue($this->wp->GetDBValue("3"), ccsInteger) . "";
        $this->SQL = "SELECT alumno.nombre AS alumno_nombre, descripcion, instructor.nombre AS instructor_nombre, asistenciaalumno.*\n" .
