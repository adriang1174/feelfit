...
//Prepare Method @2-17F69A4B
    function Prepare()
    {
        global $CCSLocales;
        global $DefaultDateFormat;
        $this->wp = new clsSQLParameters($this->ErrorBlock);
        $this->wp->AddParameter("1", "urls_idAlumno", ccsText, "", "", $this->Parameters["urls_idAlumno"], "", false);
        $this->wp->AddParameter("2", "urls_anio", ccsText, "", "", $this->Parameters["urls_anio"], "", false);
        $this->wp->AddParameter("3", "urls_mes", ccsText, "", "", $this->Parameters["urls_mes"], "", false);
    }
//End Prepare Method

//Open Method @2-26B4CC48
    function Open()
    {
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeBuildSelect", $this->Parent);
        $this->SQL = "SELECT instructor.nombre AS instructor_nombre, \n" .
        ...
        "WHERE ( tipomov = 'C' )\n" .
        "AND movimientofacturacion.idAlumno = '" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "' and length('" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "') > 0\n" .
        "AND movimientofacturacion.anio LIKE '%" . $this->SQLValue($this->wp->GetDBValue("2"), ccsText) . "%'\n" .
        "AND ( lpad(movimientofacturacion.mes,2,'0') like '%" . $this->SQLValue($this->wp->GetDBValue("3"), ccsText) . "%' ) \n" .
        "union\n" .
        ...
        "movimientofacturacion.idInstructor = instructor.id\n" .
        "WHERE ( tipomov = 'C' )\n" .
        "AND movimientofacturacion.idAlumno like '%' and length('" . $this->SQLValue($this->wp->GetDBValue("1"), ccsText) . "') = 0\n" .
        "AND movimientofacturacion.anio LIKE '%" . $this->SQLValue($this->wp->GetDBValue("2"), ccsText) . "%'\n" .
        "AND ( lpad(movimientofacturacion.mes,2,'0') like '%" . $this->SQLValue($this->wp->GetDBValue("3"), ccsText) . "%' )  {SQL_OrderBy}";
        $this->CCSEventResult = CCGetEvent($this->CCSEvents, "BeforeExecuteSelect", $this->Parent);
        //$this->query(CCBuildSQL($this->SQL, $this->Where, "2" ));
		//$this->query(CCBuildSQL($this->SQL, $this->Where, "idInstructor asc" .  ($this->Order ? ", " . $this->Order: "")));
		$this->query(CCBuildSQL($this->SQL, $this->Where, "idInstructor asc" ));
