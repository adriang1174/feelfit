<Page id="1" templateExtension="html" relativePath="." fullRelativePath="." secured="True" urlType="Relative" isIncluded="False" SSLAccess="False" isService="False" cachingEnabled="False" cachingDuration="1 minutes" wizardTheme="{CCS_Style}" wizardThemeVersion="3.0" needGeneration="0" accessDeniedPage="denegado.ccp">
	<Components>
		<Report id="2" secured="False" enablePrint="False" showMode="Web" sourceType="SQL" returnValueType="Number" linesPerWebPage="40" linesPerPhysicalPage="50" connection="Connection1" dataSource="SELECT instructor.nombre AS instructor_nombre, 
movimientofacturacion.id,
movimientofacturacion.tipomov,
movimientofacturacion.mes,
movimientofacturacion.anio,
movimientofacturacion.importe,
movimientofacturacion.idAlumno,
movimientofacturacion.idTurno,
movimientofacturacion.idInstructor,
...
WHERE ( tipomov = 'C' )
AND movimientofacturacion.idAlumno = '{s_idAlumno}' and length('{s_idAlumno}') &gt; 0
AND movimientofacturacion.anio LIKE '%{s_anio}%'
AND ( lpad(movimientofacturacion.mes,2,'0') like '%{s_mes}%' ) 
union
SELECT instructor.nombre AS instructor_nombre, 
movimientofacturacion.id,
movimientofacturacion.tipomov,
movimientofacturacion.mes,
movimientofacturacion.anio,
...
movimientofacturacion.idInstructor = instructor.id
WHERE ( tipomov = 'C' )
AND movimientofacturacion.idAlumno like '%' and length('{s_idAlumno}') = 0
AND movimientofacturacion.anio LIKE '%{s_anio}%'
AND ( lpad(movimientofacturacion.mes,2,'0') like '%{s_mes}%' ) 
ORDER BY 2
" name="movimientofacturacion" orderBy="movimientofacturacion.id" pageSizeLimit="100" wizardCaption=" Movimientofacturacion " wizardLayoutType="GroupLeftAbove" activeCollection="SQLParameters" parameterTypeListName="ParameterTypeList">
			<Components>
			...
			<SPParameters/>
			<SQLParameters>
				<SQLParameter id="64" parameterType="URL" variable="s_idAlumno" dataType="Text" parameterSource="s_idAlumno"/>
				<SQLParameter id="65" parameterType="URL" variable="s_anio" dataType="Text" parameterSource="s_anio" designDefaultValue="2013"/>
				<SQLParameter id="66" variable="s_mes" parameterType="URL" dataType="Text" parameterSource="s_mes" designDefaultValue="4"/>
			</SQLParameters>
			<ReportGroups>
